...
{
    DEBUG_PRINT("Found name:" << found_name << std::endl);
...
)
{
        ...
        ++funcname_itr) 
    {
        ...
        if (funcname_itr != addr_to_func[searchedAddr].begin()) {
            DEBUG_PRINT(">[*][" << std::hex << searchedAddr << "] " << found_func.toString() << std::endl);
        }
        DEBUG_PRINT("[+] Found the name at: " << std::hex << name_offset << std::endl);
#endif
        ...
        DEBUG_PRINT("[+] Wrote found to offset: " << std::hex << call_via_ptr << std::endl);
        is_name_saved = true;
        break;
    }
    //name not found or could not be saved - fill the ordinal instead:
    if (!is_name_saved && lastOrdinal != 0) {
        DEBUG_PRINT("[+] Filling ordinal: " << lastOrdinal << std::endl);
...
bool ImportsUneraser::writeFoundFunction(IMAGE_THUNK_DATA_T* desc, const FIELD_T ordinal_flag, const ExportedFunc &foundFunc)
{
    if (foundFunc.isByOrdinal) {
        ...
        *by_ord = ordinal;
        DEBUG_PRINT("[+] Saved ordinal" << std::endl);
        return true;
    }
    ...
    if (is_nameptr_valid) {
        ...
        DEBUG_PRINT("[+] Saved name" << std::endl);
        return true;
    }
    return false;
}
...
)
{
        ...
        )
    {
        ...
        DEBUG_PRINT("[*][" << std::hex << searchedAddr << "] " << funcname_itr->toString() << std::endl);
...
bool ImportsUneraser::uneraseDllImports(IN OUT IMAGE_IMPORT_DESCRIPTOR* lib_desc, IN ImportedDllCoverage &dllCoverage, OUT OPTIONAL ImpsNotCovered* notCovered)
{
    ...
    if (!is_filled) {
        DEBUG_PRINT("[-] Could not fill some import names!" << std::endl);
        return false;
    }
    return is_filled;
}
